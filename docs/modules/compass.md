...
3. **Real 3D** (`CompassMascot3D.tsx`, React Three Fiber) — Abishek's call once (2) kept losing its hands. First pass: torso/face/limbs as independent primitives (torus + straight cylinders + spheres) touching at their edges. **Abishek reviewed a screenshot and called it out directly as low-quality** — limbs read as disconnected rods clipped through the body with a visible seam, hands were a bare sphere with no fingers, the pose looked broken rather than a wave, and the single hard clearcoat highlight looked like a plastic sticker rather than soft toy shading. Rebuilt properly rather than nudged:
   - **Real joints, not single rotated rods.** A `Bone(from, to, radiusStart, radiusEnd, color)` helper builds a cylinder oriented via `THREE.Quaternion().setFromUnitVectors()` between two 3D points — arms are now shoulder joint (sphere, deliberately positioned *inside* the torus's outer surface so it overlaps and hides the seam) → upper-arm bone → elbow joint (sphere) → forearm bone → hand, instead of one straight cylinder punched through the ring at an angle. Legs got the same treatment (hip joint → tapered bone → foot).
   - **An actual hand**, not a sphere: a flattened palm plus four individually-oriented finger capsules fanned by angle (`Bone` reused at finger scale) plus a separately-angled thumb.
   - **Softer, more toy-like materials**: `clearcoat` lowered and `clearcoatRoughness` raised (a soft sheen instead of one hard white hotspot), plus a proper light rig — `hemisphereLight` (sky/ground bounce color, free, no HDRI fetch) + ambient + a directional key/fill pair + a rim point light — instead of the earlier bare 3-light setup.
   - Animates via `useFrame`: idle sway/bob, a talking bounce + gentle arm sway (now just a small oscillation of the whole per-side arm group, since the joint chain itself carries the pose), and a "thinking" state that spins only the nose/needle around its own base rather than the whole body — a flat torus rotated on its own Y axis passes edge-on into a near-invisible sliver partway through every turn.
   - Camera framing (`position` + `fov`) and the character's vertical recenter offset were re-tuned twice for this pose — once when the joint-based arms changed the bounding box, and again when the reaching hands overlapped the landing hero's badge text at the larger stage size. Get this wrong and it's the exact same "hands cut off" symptom as (2), just from frustum clipping instead of a 2D crop.
