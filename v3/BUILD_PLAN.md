...
- 2026-09-13 (post-P7 polish, round 5): **Pose, fingers, and nose fixed on the rigged 3D mascot** after Abishek reviewed a live screenshot. Arms changed from a permanently raised "jazz hands" wave to a relaxed hang at the side (redefined the shoulder/elbow/hand joint points) — a genuinely casual resting pose rather than a constant gesture. This shrank the bounding box a lot, so camera distance/recenter/contact-shadow position all needed re-tuning again. Doing so surfaced a real bug: fingers fan around each hand's own local +Y axis, and the hand was never oriented to the forearm's actual direction — invisible while arms pointed up, very visible once they pointed down (fingers aimed back into the arm instead of away from it). Fixed by orienting the hand with the same quaternion-alignment trick already used for bones. Separately thinned the fingers with rounded fingertip caps (were reading as "pipes"), and rebuilt the nose as a short cone flipped 180Β° on one axis instead of a long two-axis-tilted spike that looked disconnected from the face. Ported identically into `landing.html`'s vanilla copy in the same pass.

- 2026-09-13 (post-P7 polish, round 4): **Rebuilt the 3D mascot's rigging and materials — Abishek called the first pass low-quality and was right.** Reviewing a screenshot, he flagged: arms as disconnected rods clipping through the ring with a visible seam, hands with no fingers (a bare sphere), an arm pose that read as broken rather than raised/waving, and one hard clearcoat highlight looking like a plastic sticker. Root cause: independent primitives touching at their edges, not real joints. Rebuilt with a `Bone(from, to, ...)` helper (a cylinder oriented via quaternion between two 3D points) so arms are now shoulder joint → upper-arm → elbow joint → forearm → an actual hand (flattened palm + four individually-angled fingers + a thumb), with the shoulder sphere deliberately overlapping the torus's surface to hide the seam. Softened materials (lower clearcoat, higher clearcoatRoughness) and replaced the bare 3-light rig with a hemisphere light (sky/ground bounce, still no HDRI/CDN dependency) plus a key/fill/rim setup. Ported the identical redesign into `landing.html`'s vanilla-JS copy by hand. Camera distance and a couple of joint positions (foot-to-ankle seam) were re-tuned twice more after the pose's bounding box changed — same discipline as before: change the geometry, then re-check framing at every size the mascot actually renders at (toggle button, panel, composer icon, chat empty-state, landing hero), don't assume it still fits.

- 2026-09-13 (post-P7 polish, round 3): **Replaced the mascot's flat-PNG implementation with a real 3D one.** Abishek flagged that the `/compass` chat page's small avatars (composer icon, per-message avatars) were losing their hands — the single hero image, scaled/cropped into different container sizes, kept clipping them. Rather than patch the crop again, rebuilt the mascot in React Three Fiber (`CompassMascot3D.tsx`): every part is a primitive Three.js geometry (torus/sphere/cone/cylinder/capsule) with physical materials, lit by a manual light rig. `CompassAvatar.tsx` (the PNG-based component) and its now-unused CSS keyframes deleted. Two real bugs caught during this build, not just styled around: (1) drei's `<Environment>` fetches an HDRI from a remote CDN and left the entire scene blank (stuck on `<Suspense>`'s `null` fallback) since that fetch couldn't complete here — replaced with manual lights, no network dependency. (2) The arms/legs were positioned outside the camera's frustum (clipped) — same "hands missing" symptom as the PNG version, just a different root cause; fixed by widening the camera distance/FOV and recentering the character vertically. Also found mid-build: spinning the whole body on the Y axis for "thinking" made the flat torus pass edge-on into a near-invisible sliver every rotation — fixed by spinning only the nose/needle instead. `public/landing.html`'s hero was ported to the same 3D character by hand in vanilla Three.js (no React there) once Abishek asked directly whether it still used the old image — caught two more real bugs doing it: a wrong cdnjs version-string format for the three.js URL (404, silently blank stage, no console error since the "library not loaded" guard couldn't tell a failed CDN fetch from "not needed"; fixed the URL and made the guard loud) and an apparent canvas overflow that turned out to be the raised hands rendering close enough to the frame edge to visually collide with a badge (fixed by pulling the camera back, not by touching the — correct — CSS layout). `frontend/public/mascot/compass-idle.png` is now fully unused and deleted.

- 2026-09-13 (post-P7 polish, round 2): Three changes from Abishek's continued live review. (1) **Removed the "Classic Ask" nav tab** — `/ask` still works by direct URL, just unlinked. (2) **Forced light-mode only, everywhere** — the app previously had a `prefers-color-scheme: dark` variant that kicked in on a dark OS/browser; Abishek was seeing that unexpectedly and wanted the single designed (light) look, matching the Figma reference, with no dark variant at all. Removed the dark media query block from `globals.css` entirely and set `color-scheme: light` on `:root` (also added to `landing.html`, which never had a dark block to begin with) so no OS/browser dark preference can override it. (3) **Landing page is now the app's root**, reversing the P2 decision: `/` is a route handler (`app/route.ts`) serving `public/landing.html` directly (a route handler and a page can't share one segment, hence `route.ts` not `page.tsx`), the dashboard moved from `/` to `/dashboard` as a real page, and `/landing` is kept only as a redirect to `/` for old links. `TopNav`'s brand mark, its "Dashboard" nav item, and the landing page's own CTAs all now point at `/dashboard`. Also (separately, same session): neutralized dashboard chrome (background/borders/gridlines/muted-text) away from the purple tint that compounded across the dense grid — see the entry above — and removed the redundant per-tile "Ask AI about this" buttons now that the floating Compass widget is a global entry point (deleted the now-orphaned `AskAiButton.tsx`), added a logomark + display font for the "Compass" wordmark, and generated an actual compass-themed favicon (there wasn't one before — just Next's default scaffold icon).
