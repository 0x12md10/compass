...
- **Landing/marketing page is the public root (`/`)**, with the dashboard at `/dashboard` — reversed from this doc's original call (landing at `/landing`, dashboard as home) once the landing page actually existed and Abishek wanted it to be the front door instead. See `v3/BUILD_PLAN.md`'s log for the routing history. Still no logged-out/logged-in split — `/dashboard` is just another public route, same as before.
